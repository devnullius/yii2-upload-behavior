...
class GD extends PHPThumb
{
    ...
    public function adaptiveResizePercent(int $width, int $height, int $percent = 50): GD
    {
        ...
        if ($this->currentDimensions['width'] > $this->maxWidth) {
            ...
            $cropX = (int)(($percent / 100) * $maxCropX);

        } elseif ($this->currentDimensions['height'] > $this->maxHeight) {
            // Image is portrait
            $maxCropY = $this->currentDimensions['height'] - $this->maxHeight;
            $cropY = (int)(($percent / 100) * $maxCropY);
        }
        ...
        if ($this->currentDimensions['width'] > $this->maxWidth) {
            ...
            switch ($quadrant) {
                case 'L':
                    ...
                    break;
                case 'R':
                    $cropX = (int)(($this->currentDimensions['width'] - $this->maxWidth));
                    break;
                case 'C':
                default:
                    $cropX = (int)(($this->currentDimensions['width'] - $this->maxWidth) / 2);
            ...
            switch ($quadrant) {
                case 'T':
                    ...
                    break;
                case 'B':
                    $cropY = (int)(($this->currentDimensions['height'] - $this->maxHeight));
                    break;
                case 'C':
                default:
                    $cropY = (int)(($this->currentDimensions['height'] - $this->maxHeight) / 2);
    ...
    protected function determineFormat(): void
    {
        ...
        if ($formatInfo === false) {
            if ($this->remoteImage) {
                throw new RuntimeException("Could not determine format of remote image: {$this->fileName}");
            }

            throw new RuntimeException("File is not a valid image: {$this->fileName}");
        }
        ...
        switch ($mimeType) {
            ...
            case 'image/png':
                ...
                break;
            default:
                throw new RuntimeException("Image format not supported: {$mimeType}");
        }
    }
